...
func FormatEvent(output *output.ResultEvent) []byte {
	builder := &bytes.Buffer{}
	builder.WriteRune('[')
	builder.WriteString(aurora.Cyan(time.Now().Format("2006-01-02 15:04:05")).String())
	builder.WriteString("] ")

	builder.WriteRune('[')
	builder.WriteString(aurora.BrightGreen(output.TemplateID).String())

	if output.MatcherName != "" {
		builder.WriteString(":")
		builder.WriteString(aurora.BrightGreen(output.MatcherName).Bold().String())
	} else if output.ExtractorName != "" {
		builder.WriteString(":")
		builder.WriteString(aurora.BrightGreen(output.ExtractorName).Bold().String())
	}

	// match status
	builder.WriteString("] [")
	if !output.MatcherStatus {
		builder.WriteString(aurora.Red("failed").String())
	} else {
		builder.WriteString(aurora.Green("matched").String())
	}

	builder.WriteString("] [")
	builder.WriteString(aurora.BrightBlue(output.Type).String())
	builder.WriteString("] ")

	builder.WriteString("[")
	builder.WriteString(GetColorSeverity(output.Info.SeverityHolder.Severity))
	builder.WriteString("] ")
	...
	if len(output.Metadata) > 0 {
		...
		for _, name := range mapsutil.GetSortedKeys(output.Metadata) {
			...
			builder.WriteString(aurora.BrightYellow(name).String())
			builder.WriteRune('=')
			builder.WriteString(aurora.BrightYellow(strconv.QuoteToASCII(types.ToString(value))).String())
		}
		builder.WriteString("]")
	}
	...
	if output.IsFuzzingResult {
		if output.FuzzingParameter != "" {
			...
			builder.WriteRune(':')
			builder.WriteString(aurora.BrightMagenta(output.FuzzingParameter).String())
			builder.WriteString("]")
		}
